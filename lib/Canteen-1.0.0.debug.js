(function() {
    ...
    Debug.minLogLevel = Debug.GENERAL, Debug.enabled = !0, Debug.output = null, Debug.log = function(params) {
        ...
        error.stackTrace) for (var j = 0; error.stackTrace.length > j; j++) result += "	 #" + (j + 1) + ". " + error.stackTrace[j];
        Debug.error(result);
    ...
    Forms.FEEDBACK_SUCCESS = "formSuccess", Forms.setup = function(site) {
        ...
        for (i = 0; len > i; i++) $("input[type='" + types[i] + "']").removeClass(types[i]).addClass(types[i]).hover(onRollOver, onRollOut).keydown(onKeyDown);
        $("form").each(function() {
            var form = $(this);
        ...
        formElements.each(function() {
            ...
            }), formElements.enabled()), undefined);
        }, function(response) {
        ...
        params.async = "true", Debug.log(params), $.post(window.location.href, params, function(response) {
            ...
            "fatalError" == response.type ? (Debug.fatalError(response.errors), undefined) : (response.ifError ? errorCallback !== undefined && errorCallback(response) : successCallback !== undefined && successCallback(response), 
            undefined));
        });
    }, $.fn.enabled = function(enabled) {
        return enabled = enabled === undefined ? !0 : enabled, this.each(function() {
            $(this).prop("disabled", !enabled).removeClass(Forms.DISABLED), enabled || $(this).addClass(Forms.DISABLED);
        });
    ...
    p.gatewayUrl = null, p.initialize = function(gatewayUrl, callback) {
        ...
        Debug.log(url), $.get(url, function(response) {
            ...
            return "fatalError" == data.type ? (Debug.fatalError(data.errors), callback(null), 
            undefined) : (callback(data), undefined);
        });
    }, p.destroy = function() {
        p = null;
    ...
    Site.VERSION = "1.0.0", p.initialize = function(options) {
        ...
        $(opts.siteLoadingId).addClass(opts.loadingClass), _currentId = 1, _currentState = Canteen.settings.uriRequest, 
        this._enableHistory(!0), this._fixInternalLinks(), Debug.enabled = Canteen.settings.debug, 
        this.parameters = LocationUtils.getParameters(), this.gateway = new Gateway(Canteen.settings.gatewayPath, this._gatewayReady.bind(this));
